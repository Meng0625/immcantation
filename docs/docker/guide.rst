...
To invoke a shell session inside the container::
    ...
    # Docker command
    docker run -it kleinstein/immcantation:1.6.0 bash

    # Singularity command
    singularity shell immcantation-1.6.0.img
...
``/data``::
    ...
    # Docker command
    docker run -it -v $HOME/project:/data:z kleinstein/immcantation:1.6.0 bash

    # Singularity command
    singularity shell -B $HOME/project:/data immcantation-1.6.0.img
...
``/data``::
    ...
    # Docker command
    docker run -v $HOME/project:/data:z kleinstein/immcantation:1.6.0 versions report

    # Singularity command
    singularity exec -B $HOME/project:/data immcantation-1.6.0.img versions report
...
.. code-block:: shell
    ...
    # Docker command
    docker run -v $DATA_DIR:/data:z kleinstein/immcantation:1.6.0 presto-abseq \
        -1 $READS_R1 -2 $READS_R2 -y $YAML -n $SAMPLE_NAME -o $OUT_DIR -p $NPROC \
        | tee run_presto.out

    # Singularity command
    singularity exec -B $DATA_DIR:/data immcantation-1.6.0.img presto-abseq \
        -1 $READS_R1 -2 $READS_R2 -y $YAML -n $SAMPLE_NAME -o $OUT_DIR -p $NPROC \
        | tee run_presto.out
...
.. code-block:: shell
    ...
    docker run -v $DATA_DIR:/data:z kleinstein/immcantation:1.6.0 changeo-igblast \
        -s $READS -n $SAMPLE_NAME -o $OUT_DIR -p $NPROC \
        | tee run_igblast.out

    # Singularity command
    singularity exec -B $DATA_DIR:/data immcantation-1.6.0.img changeo-igblast \
        -s $READS -n $SAMPLE_NAME -o $OUT_DIR -p $NPROC \
        | tee run_igblast.out
...
.. code-block:: shell
    ...
    # Run pipeline in docker image
    docker run -v $DATA_DIR:/data:z kleinstein/immcantation:1.6.0 tigger-genotype \
        -d $DB -n $SAMPLE_NAME -o $OUT_DIR -p $NPROC \
        | tee run_genotype.out

    # Singularity command
    singularity exec -B $DATA_DIR:/data immcantation-1.6.0.img tigger-genotype \
        -d $DB -n $SAMPLE_NAME -o $OUT_DIR -p $NPROC \
        | tee run_genotype.out
...
.. code-block:: shell
    ...
    # Run pipeline in docker image
    docker run -v $DATA_DIR:/data:z kleinstein/immcantation:1.6.0 shazam-threshold \
        -d $DB -n $SAMPLE_NAME -o $OUT_DIR -p $NPROC \
        | tee run_threshold.out

    # Singularity command
    singularity exec -B $DATA_DIR:/data immcantation-1.6.0.img shazam-threshold \
        -d $DB -n $SAMPLE_NAME -o $OUT_DIR -p $NPROC \
        | tee run_threshold.out
...
.. code-block:: shell
    ...
    # Run pipeline in docker image
    docker run -v $DATA_DIR:/data:z kleinstein/immcantation:1.6.0 changeo-clone \
        -d $DB -x $DIST -n $SAMPLE_NAME -o $OUT_DIR -p $NPROC \
        | tee run_clone.out

    # Singularity command
    singularity exec -B $DATA_DIR:/data immcantation-1.6.0.img changeo-clone \
        -d $DB -x $DIST -n $SAMPLE_NAME -o $OUT_DIR -p $NPROC \
        | tee run_clone.out
...
.. code-block:: shell
    ...
    # Run pipeline in docker image
    docker run -v $DATA_DIR:/data:z kleinstein/immcantation:1.6.0 preprocess-phix \
        -s $READS -o $OUT_DIR -p $NPROC \
        | tee run_phix.out

    # Singularity command
    singularity exec -B $DATA_DIR:/data immcantation-1.6.0.img preprocess-phix \
        -s $READS -o $OUT_DIR -p $NPROC \
        | tee run_phix.out
